  ...
  create  table "quasar"."public"."posts__dbt_tmp"
  as (
    SELECT
	a.name AS "action",
	pd.action_id,
...
FROM "quasar"."ft_rogue_dosomething_rogue_qa"."posts" pd
INNER JOIN "quasar"."public"."signups" s
	ON pd.signup_id = s.id
LEFT JOIN "quasar"."public"."turbovote" tv
...
LEFT JOIN
	(SELECT
		...
		FROM "quasar"."public"."rock_the_vote" r
	) rtv
	ON rtv.post_id::bigint = pd.id::bigint
LEFT JOIN "quasar"."ft_rogue_dosomething_rogue_qa"."actions" a
