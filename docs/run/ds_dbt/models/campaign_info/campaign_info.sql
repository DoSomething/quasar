  ...
  create  table "quasar"."public"."campaign_info__dbt_tmp"
  as (
    WITH
...
campaign_action AS (
  ...
  FROM "quasar"."public"."post_actions"
...
SELECT 
  ...
  cm.display_referral_page as contentful_RAF_flag,
	...
	s.scholarship,
	p.post_types
FROM "quasar"."ft_rogue_dosomething_rogue_qa"."campaigns" c
LEFT JOIN "quasar"."public"."campaign_info_ashes_snapshot" i ON i.campaign_run_id = c.campaign_run_id
...
LEFT JOIN "quasar"."public_intermediate"."contentful_metadata" cm on c.id = cm.legacy_campaign_id
