  ...
  create  table "quasar"."public"."user_rb_summary__dbt_tmp"
  as (
...
WITH rbs_all AS (
	SELECT
		...
		END AS scholarship
	FROM
		"quasar"."public"."reportbacks" r
		JOIN "quasar"."public"."posts" p ON (r.post_id = p.id)
		JOIN "quasar"."public"."post_actions" pa ON (p.action_id = pa.id)
	WHERE
		r.signup_id > 0
	ORDER BY
		r.signup_id
